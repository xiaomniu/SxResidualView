...
#define GLCORE_THIS this->m_pGlobalGLCore->
...
int CSoilTool::isAtLeastGL3()
{

	if ( SOIL_CAPABILITY_UNKNOWN == is_gl3 )
	{
        const char * verstr	= (const char *) GLCORE_THIS glGetString( GL_VERSION );
...
void * CSoilTool::SOIL_GL_GetProcAddress(const char *proc)
{
	void *func = NULL;
	...
	func =  GetProcAddress(openglModule, proc );//func =  wglGetProcAddress( proc );
...
int CSoilTool::SOIL_GL_ExtensionSupported(const char *extension)
{
	...
	if ( isAtLeastGL3() )
	{
		...
		#endif
        GLCORE_THIS glGetIntegerv(GL_NUM_EXTENSIONS, &num_exts);
    ...
    extensions = (const char *) GLCORE_THIS glGetString(GL_EXTENSIONS);
...
unsigned int
    CSoilTool::SOIL_load_OGL_texture
	(
		const char *filename,
		int force_channels,
...
unsigned int
    CSoilTool::SOIL_load_OGL_HDR_texture
	(
		const char *filename,
		int fake_HDR_format,
		int rescale_to_max,
...
unsigned int
    CSoilTool::SOIL_load_OGL_texture_from_memory
	(
...
unsigned int
    CSoilTool::SOIL_load_OGL_cubemap
	(
...
unsigned int
    CSoilTool::SOIL_load_OGL_cubemap_from_memory
	(
...
unsigned int
    CSoilTool::SOIL_load_OGL_single_cubemap
	(
		const char *filename,
...
unsigned int
    CSoilTool::SOIL_load_OGL_single_cubemap_from_memory
	(
		const unsigned char *const buffer,
		int buffer_length,
		const char face_order[6],
...
unsigned int
    CSoilTool::SOIL_create_OGL_single_cubemap
	(
...
unsigned int
    CSoilTool::SOIL_create_OGL_texture
	(
		const unsigned char *const data,
		int *width, int *height, int channels,
		unsigned int reuse_texture_ID,
...
{
	/*	check for errors	*/
    GLenum err_code = GLCORE_THIS glGetError();
	while( GL_NO_ERROR != err_code )
	{
		printf( "OpenGL Error @ %s: %i", calling_location, err_code );
        err_code = GLCORE_THIS glGetError();
	}
}
#else
void check_for_GL_errors( const char *calling_location )
...
void CSoilTool::createMipmaps(const unsigned char *const img,
		int width, int height, int channels,
		unsigned int flags,
...
unsigned int
    CSoilTool::SOIL_internal_create_OGL_texture
	(
		const unsigned char *const data,
		int *width, int *height, int channels,
		unsigned int reuse_texture_ID,
	...
	)
{
    ...
    GLCORE_THIS glGetIntegerv( texture_check_size_enum, &max_supported_size );
	...
	if( tex_id == 0 )
	{
        GLCORE_THIS glGenTextures( 1, &tex_id );
	}
	...
	if( tex_id )
	{
        ...
        GLCORE_THIS glBindTexture( opengl_texture_type, tex_id );
		check_for_GL_errors( "glBindTexture" );

		/* set the unpack aligment */
        GLCORE_THIS glGetIntegerv(GL_UNPACK_ALIGNMENT, &unpack_aligment);
		if ( 1 != unpack_aligment )
		{
            GLCORE_THIS glPixelStorei(GL_UNPACK_ALIGNMENT,1);
		}
		...
		if( flags & SOIL_FLAG_MIPMAPS || flags & SOIL_FLAG_GL_MIPMAPS )
		{
            ...
            GLCORE_THIS glTexParameteri( opengl_texture_type, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
            GLCORE_THIS glTexParameteri( opengl_texture_type, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR );
			check_for_GL_errors( "GL_TEXTURE_MIN/MAG_FILTER" );
		} else
		{
			/*	instruct OpenGL _NOT_ to use the MIPmaps	*/
            GLCORE_THIS glTexParameteri( opengl_texture_type, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
            GLCORE_THIS glTexParameteri( opengl_texture_type, GL_TEXTURE_MIN_FILTER, GL_LINEAR );
			check_for_GL_errors( "GL_TEXTURE_MIN/MAG_FILTER" );
		}
		...
		if ( 1 != unpack_aligment )
		{
            GLCORE_THIS glPixelStorei(GL_UNPACK_ALIGNMENT, unpack_aligment);
		}

		/*	does the user want clamping, or wrapping?	*/
		if( flags & SOIL_FLAG_TEXTURE_REPEATS )
		{
            GLCORE_THIS glTexParameteri( opengl_texture_type, GL_TEXTURE_WRAP_S, GL_REPEAT );
            GLCORE_THIS glTexParameteri( opengl_texture_type, GL_TEXTURE_WRAP_T, GL_REPEAT );
			if( opengl_texture_type == SOIL_TEXTURE_CUBE_MAP )
			{
				/*	SOIL_TEXTURE_WRAP_R is invalid if cubemaps aren't supported	*/
                GLCORE_THIS glTexParameteri( opengl_texture_type, SOIL_TEXTURE_WRAP_R, GL_REPEAT );
			}
		...
		} else
		{
            ...
            GLCORE_THIS glTexParameteri( opengl_texture_type, GL_TEXTURE_WRAP_S, clamp_mode );
            GLCORE_THIS glTexParameteri( opengl_texture_type, GL_TEXTURE_WRAP_T, clamp_mode );
			if( opengl_texture_type == SOIL_TEXTURE_CUBE_MAP )
			{
				/*	SOIL_TEXTURE_WRAP_R is invalid if cubemaps aren't supported	*/
                GLCORE_THIS glTexParameteri( opengl_texture_type, SOIL_TEXTURE_WRAP_R, clamp_mode );
			}
			check_for_GL_errors( "GL_TEXTURE_WRAP_*" );
		}
		/*	done	*/
...
int
    CSoilTool::SOIL_save_screenshot
	(
		const char *filename,
		int image_type,
		int x, int y,
	...
	)
{
    ...
    GLCORE_THIS glGetIntegerv(GL_PACK_ALIGNMENT, &pack_aligment);
	if ( 1 != pack_aligment )
	{
        GLCORE_THIS glPixelStorei(GL_PACK_ALIGNMENT,1);
	}

	/*  Get the data from OpenGL	*/
	pixel_data = (unsigned char*)malloc( 3*width*height );
    GLCORE_THIS glReadPixels (x, y, width, height, GL_RGB, GL_UNSIGNED_BYTE, pixel_data);

	if ( 1 != pack_aligment )
	{
        GLCORE_THIS glPixelStorei(GL_PACK_ALIGNMENT, pack_aligment);
	}
...
unsigned char*
    CSoilTool::SOIL_load_image_from_memory
	(
		const unsigned char *const buffer,
		int buffer_length,
		int *width, int *height, int *channels,
...
int
    CSoilTool::SOIL_save_image_quality
	(
		const char *filename,
		int image_type,
		int width, int height, int channels,
...
void
    CSoilTool::SOIL_free_image_data
	(
...
unsigned int CSoilTool::SOIL_direct_load_DDS_from_memory(
		const unsigned char *const buffer,
		int buffer_length,
		unsigned int reuse_texture_ID,
		int flags,
		int loading_as_cubemap)
{
	...
	tex_ID = reuse_texture_ID;
    if( tex_ID == 0 ) { GLCORE_THIS glGenTextures( 1, &tex_ID ); }
	/*  bind an OpenGL texture ID	*/
    GLCORE_THIS glBindTexture( opengl_texture_type, tex_ID );
	...
	for( cf_target = ogl_target_start; cf_target <= ogl_target_end; ++cf_target )
	{
		if( buffer_index + DDS_full_size <= (unsigned int)buffer_length )
		{
			...
			if( uncompressed )
			{
                ...
                GLCORE_THIS glTexImage2D( cf_target, 0, internal_format, width, height, 0, internal_format, format_type, DDS_data );
			}
			else
			{
			...
			for( int i = 1; i <= mipmaps; ++i )
			{
				...
				if( uncompressed )
				{
                    ...
                    GLCORE_THIS glTexImage2D( cf_target, i, internal_format, w, h, 0, internal_format, format_type,
		...
		{
            GLCORE_THIS glDeleteTextures( 1, &tex_ID );
			tex_ID = 0;
...
unsigned int CSoilTool::SOIL_direct_load_DDS(
		const char *filename,
		unsigned int reuse_texture_ID,
		int flags,
		int loading_as_cubemap )
...
unsigned int CSoilTool::SOIL_direct_load_PVR_from_memory(
		const unsigned char *const buffer,
		int buffer_length,
		unsigned int reuse_texture_ID,
		int flags,
		int loading_as_cubemap )
{
	...
	if ( 1 != unpack_aligment )
	{
        GLCORE_THIS glPixelStorei(GL_UNPACK_ALIGNMENT,1);				// Never have row-aligned in headers
	}
	...
	for(i=0; i<num_surfs; i++) {
		...
		for(mipmap_level = 0; mipmap_level <= mipmaps; width = _MAX(width/2, (unsigned int)1), height = _MAX(height/2, (unsigned int)1), mipmap_level++ ) {
			...
			if( is_compressed_format ) {
				...
				if ( loading_as_cubemap ) {
                    GLCORE_THIS glTexImage2D( SOIL_TEXTURE_CUBE_MAP_POSITIVE_X + i, mipmap_level, PVR_type, width, height, 0, PVR_type, PVR_format, cur_texture_ptr );
				} else {
                    GLCORE_THIS glTexImage2D( opengl_texture_type, mipmap_level, PVR_type, width, height, 0, PVR_type, PVR_format, cur_texture_ptr );
				}
			}
...
unsigned int CSoilTool::SOIL_direct_load_PVR(
		const char *filename,
		unsigned int reuse_texture_ID,
		int flags,
		int loading_as_cubemap )
...
unsigned int CSoilTool::SOIL_direct_load_ETC1_from_memory(
		const unsigned char *const buffer,
		int buffer_length,
		unsigned int reuse_texture_ID,
		int flags )
...
unsigned int CSoilTool::SOIL_direct_load_ETC1(const char *filename,
		unsigned int reuse_texture_ID,
		int flags )
{
	FILE *f;
...
int CSoilTool::query_NPOT_capability( void )
{
...
int CSoilTool::query_tex_rectangle_capability( void )
{
	/*	check for the capability	*/
...
int CSoilTool::query_cubemap_capability( void )
{
	/*	check for the capability	*/
...
P_SOIL_GLCOMPRESSEDTEXIMAGE2DPROC CSoilTool::get_glCompressedTexImage2D_addr()
{
...
int CSoilTool::query_DXT_capability( void )
{
	/*	check for the capability	*/
...
int CSoilTool::query_3Dc_capability(void) {
	/*	check for the capability	*/
...
int CSoilTool::query_PVR_capability( void )
{
	/*	check for the capability	*/
...
int CSoilTool::query_BGRA8888_capability( void )
{
	/*	check for the capability	*/
...
int CSoilTool::query_sRGB_capability( void )
{
...
int CSoilTool::query_ETC1_capability( void )
{
	/*	check for the capability	*/
...
int CSoilTool::query_gen_mipmap_capability( void )
{
	/* check for the capability   */
