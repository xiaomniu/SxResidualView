...
#include "D:/QT_CODE_PRJ/SuperXView01/Src_GlWnd/COpenGLCore.h"
extern COpenGLCore* g_GlobalGLCore;
...
static int isAtLeastGL3()
{
	...
	{
		const char * verstr	= (const char *) g_GlobalGLCore->glGetString( GL_VERSION );
...
int SOIL_GL_ExtensionSupported(const char *extension)
{
	...
	if ( isAtLeastGL3() )
	{
		...
		#endif
		g_GlobalGLCore->glGetIntegerv(GL_NUM_EXTENSIONS, &num_exts);
	...
	extensions = (const char *) g_GlobalGLCore->glGetString(GL_EXTENSIONS);
...
{
	/*	check for errors	*/
	GLenum err_code = g_GlobalGLCore->glGetError();
	while( GL_NO_ERROR != err_code )
	{
		printf( "OpenGL Error @ %s: %i", calling_location, err_code );
		err_code = g_GlobalGLCore->glGetError();
	}
}
#else
void check_for_GL_errors( const char *calling_location )
	...
	)
{
	...
	g_GlobalGLCore->glGetIntegerv( texture_check_size_enum, &max_supported_size );
	...
	if( tex_id == 0 )
	{
		g_GlobalGLCore->glGenTextures( 1, &tex_id );
	}
	...
	if( tex_id )
	{
		...
		g_GlobalGLCore->glBindTexture( opengl_texture_type, tex_id );
		check_for_GL_errors( "glBindTexture" );

		/* set the unpack aligment */
		g_GlobalGLCore->glGetIntegerv(GL_UNPACK_ALIGNMENT, &unpack_aligment);
		if ( 1 != unpack_aligment )
		{
			g_GlobalGLCore->glPixelStorei(GL_UNPACK_ALIGNMENT,1);
		}
		...
		if( flags & SOIL_FLAG_MIPMAPS || flags & SOIL_FLAG_GL_MIPMAPS )
		{
			...
			g_GlobalGLCore->glTexParameteri( opengl_texture_type, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
			g_GlobalGLCore->glTexParameteri( opengl_texture_type, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR );
			check_for_GL_errors( "GL_TEXTURE_MIN/MAG_FILTER" );
		} else
		{
			/*	instruct OpenGL _NOT_ to use the MIPmaps	*/
			g_GlobalGLCore->glTexParameteri( opengl_texture_type, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
			g_GlobalGLCore->glTexParameteri( opengl_texture_type, GL_TEXTURE_MIN_FILTER, GL_LINEAR );
			check_for_GL_errors( "GL_TEXTURE_MIN/MAG_FILTER" );
		}
		...
		if ( 1 != unpack_aligment )
		{
			g_GlobalGLCore->glPixelStorei(GL_UNPACK_ALIGNMENT, unpack_aligment);
		}

		/*	does the user want clamping, or wrapping?	*/
		if( flags & SOIL_FLAG_TEXTURE_REPEATS )
		{
			g_GlobalGLCore->glTexParameteri( opengl_texture_type, GL_TEXTURE_WRAP_S, GL_REPEAT );
			g_GlobalGLCore->glTexParameteri( opengl_texture_type, GL_TEXTURE_WRAP_T, GL_REPEAT );
			if( opengl_texture_type == SOIL_TEXTURE_CUBE_MAP )
			{
				/*	SOIL_TEXTURE_WRAP_R is invalid if cubemaps aren't supported	*/
				g_GlobalGLCore->glTexParameteri( opengl_texture_type, SOIL_TEXTURE_WRAP_R, GL_REPEAT );
			}
		...
		} else
		{
			...
			g_GlobalGLCore->glTexParameteri( opengl_texture_type, GL_TEXTURE_WRAP_S, clamp_mode );
			g_GlobalGLCore->glTexParameteri( opengl_texture_type, GL_TEXTURE_WRAP_T, clamp_mode );
			if( opengl_texture_type == SOIL_TEXTURE_CUBE_MAP )
			{
				/*	SOIL_TEXTURE_WRAP_R is invalid if cubemaps aren't supported	*/
				g_GlobalGLCore->glTexParameteri( opengl_texture_type, SOIL_TEXTURE_WRAP_R, clamp_mode );
			}
			check_for_GL_errors( "GL_TEXTURE_WRAP_*" );
		}
		/*	done	*/
	...
	)
{
	...
	g_GlobalGLCore->glGetIntegerv(GL_PACK_ALIGNMENT, &pack_aligment);
	if ( 1 != pack_aligment )
	{
		g_GlobalGLCore->glPixelStorei(GL_PACK_ALIGNMENT,1);
	}

	/*  Get the data from OpenGL	*/
	pixel_data = (unsigned char*)malloc( 3*width*height );
	g_GlobalGLCore->glReadPixels (x, y, width, height, GL_RGB, GL_UNSIGNED_BYTE, pixel_data);

	if ( 1 != pack_aligment )
	{
		g_GlobalGLCore->glPixelStorei(GL_PACK_ALIGNMENT, pack_aligment);
	}
		...
		int loading_as_cubemap)
{
	...
	tex_ID = reuse_texture_ID;
	if( tex_ID == 0 ) { g_GlobalGLCore->glGenTextures( 1, &tex_ID ); }
	/*  bind an OpenGL texture ID	*/
	g_GlobalGLCore->glBindTexture( opengl_texture_type, tex_ID );
	...
	for( cf_target = ogl_target_start; cf_target <= ogl_target_end; ++cf_target )
	{
		if( buffer_index + DDS_full_size <= (unsigned int)buffer_length )
		{
			...
			if( uncompressed )
			{
				...
				g_GlobalGLCore->glTexImage2D( cf_target, 0, internal_format, width, height, 0, internal_format, format_type, DDS_data );
			}
			else
			{
			...
			for( int i = 1; i <= mipmaps; ++i )
			{
				...
				if( uncompressed )
				{
					...
					g_GlobalGLCore->glTexImage2D( cf_target, i, internal_format, w, h, 0, internal_format, format_type,
		...
		{
			g_GlobalGLCore->glDeleteTextures( 1, &tex_ID );
			tex_ID = 0;
		...
		int loading_as_cubemap )
{
	...
	if ( 1 != unpack_aligment )
	{
		g_GlobalGLCore->glPixelStorei(GL_UNPACK_ALIGNMENT,1);				// Never have row-aligned in headers
	}
	...
	for(i=0; i<num_surfs; i++) {
		...
		for(mipmap_level = 0; mipmap_level <= mipmaps; width = _MAX(width/2, (unsigned int)1), height = _MAX(height/2, (unsigned int)1), mipmap_level++ ) {
			...
			if( is_compressed_format ) {
				...
				if ( loading_as_cubemap ) {
					g_GlobalGLCore->glTexImage2D( SOIL_TEXTURE_CUBE_MAP_POSITIVE_X + i, mipmap_level, PVR_type, width, height, 0, PVR_type, PVR_format, cur_texture_ptr );
				} else {
					g_GlobalGLCore->glTexImage2D( opengl_texture_type, mipmap_level, PVR_type, width, height, 0, PVR_type, PVR_format, cur_texture_ptr );
				}
			}
